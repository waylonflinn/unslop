...
sorts the full file list lexicographically. The default `file_root` is the
longest common parent path shared by every parsed file; all recorded paths
and `file_set` entries are relative to it. Directory inputs are flat unless
...
2. A **comment header** at the top of the file:
   ...
   `file_root` is the base all `file_set` paths are relative to.
